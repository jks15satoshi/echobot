...
from echobot.utils import confirm_intent, styledstr
...
# 一般指令事件处理
@reboot.got('confirm', prompt=styledstr('admin.reboot.prompt'))
async def handle(bot: Bot, event: Event, state: T_State) -> None:
    ...
    if confirm_intent(confirm) == 'confirm':
        ...
        await reboot.finish(styledstr('admin.reboot.cancel'))
    else:
        await reboot.finish(styledstr('admin.reboot.failed'))
...
async def _reboot(bot: Bot, event: Event) -> None:
    ...
    '''
    await bot.send(event, styledstr('admin.reboot.on_reboot'))

    try:
        await bot.set_restart()
    except (exception.NetworkError, exception.ActionFailed) as err:
        logger.error(err)
        await bot.send(event, styledstr('admin.reboot.on_err'))
